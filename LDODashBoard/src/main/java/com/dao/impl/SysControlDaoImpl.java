...
public class SysControlDaoImpl <T> implements SimpleInterfaceDao <T> {
	...
	static Logger log = Logger.getLogger(SysControlDaoImpl.class.getName());
	...
	public List<T> loadAll(String query){
		List<SysControl> listinfo  =  null;

		log.info("Executing query loadAll:" + query);

		JdbcTemplate jdbcTemplate = new JdbcTemplate(GlobalcashmanmdataSource); 

		listinfo  = jdbcTemplate.query(query,new BeanPropertyRowMapper(SysControl.class));
